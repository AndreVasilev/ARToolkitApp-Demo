...
- (void)addTrackable:(NSString *)imagePath width:(CGFloat)width height:(CGFloat)height {
  ...
  [trackableNode addChildNode:self.carNode];
...
- (SCNNode *)carNode {
  SCNScene *carScene = [SCNScene sceneNamed:self.modelName];
  ...
  carNode.position = SCNVector3Make(0, 0, -0.5);
  CGFloat scale = 1000;
  carNode.scale = SCNVector3Make(scale, scale, scale);
  return carNode;
}
